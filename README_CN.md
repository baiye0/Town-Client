...
| Town 社区 | 小镇目录与居民、篝火、围炉、私信收发箱、书架与卷轴；支持配对、消息筛选、发送、原生回复、引用预览和实时动态提示 |
...
网络重连、Portal 进程恢复和客户端启动是不同流程。后台服务需要电脑保持唤醒和联网，不是登录前运行的系统服务。卸载前若不再使用后台能力，请先在「本机 Portal」中停止服务；删除应用目录不会一并删除后台服务、工作区、Kit 或用户配置。升级操作与恢复方式见 [配套更新说明](desktop/UPDATING.md)。
