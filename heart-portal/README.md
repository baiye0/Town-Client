...
on Windows to check the supervised process. Use `heart-portal status --json` for a fresh structured connection sample, or an
MCP tool call to verify connectivity; `/api/health` is no longer available.

## Security

Portal runs as your OS user. New installations opt in to commands and executable
extensions; existing configurations retain their behavior. File tools confine
operations to the workspace, but shell commands and kits are trusted user code,
not sandboxed. Relay mode uses outbound WSS (exact loopback permits plaintext);
local TCP requires a token and defaults to loopback.

See [security and permissions](SECURITY.md) and the [runtime design and limits](docs/runtime.md)
for process cleanup, reconnect behavior, status, and update verification.
