...
pub struct ProcessManager {
    ...
    callback_slots: Arc<tokio::sync::Semaphore>,
}
...
async fn deliver_callback(cfg: CallbackConfig, session_id: String, payload: serde_json::Value) {
    ...
    for attempt in 0..CALLBACK_ATTEMPTS {
        ...
        let Ok(mut url_with_token) = url::Url::parse(&cfg.url) else {
            return;
        };
        url_with_token
            .query_pairs_mut()
...
impl ProcessManager {
    pub fn new() -> Self {
        Self {
            ...
            permits: Arc::new(tokio::sync::Semaphore::new(DEFAULT_MAX_SESSIONS)),
            max_output_bytes: DEFAULT_MAX_OUTPUT_BYTES,
            callback_config: Arc::new(Mutex::new(None)),
            callback_slots: Arc::new(tokio::sync::Semaphore::new(4)),
        }
    }

    pub(crate) fn reserve(&self) -> Result<tokio::sync::OwnedSemaphorePermit> {
        self.permits.clone().try_acquire_owned().map_err(|_| {
            anyhow::anyhow!("Maximum concurrent commands ({DEFAULT_MAX_SESSIONS}) reached")
        })
    }
    ...
    pub(crate) async fn spawn_with_shell(
        ...
        // Bound retained completed sessions as well as active commands.
        self.cleanup().await;
        ...
        let mut child = crate::child_process::ChildProcess::spawn(&mut cmd)?;
        ...
        };
        {
            ...
            while sessions.len() >= 128 {
                let oldest = sessions
                    .iter()
                    .filter_map(|(id, p)| p.exited_at.map(|at| (id.clone(), at)))
                    .min_by_key(|(_, at)| *at)
                    .map(|(id, _)| id);
            ...
            sessions.insert(session_id.clone(), proc);
        }
        ...
        let callback_slots = self.callback_slots.clone();
        ...
        tokio::spawn(async move {
            ...
            let result = tokio::select! {
                biased;
                _ = cancellation.changed() => child.terminate().await.map_err(std::io::Error::other),
                result = child.wait() => result,
            };
            cleanup_failed.store(result.is_err(), Ordering::SeqCst);
            ...
            .is_err()
            {
                ...
                warn!("session {sid_wait}: output drain timed out; readers closed");
            }
            ...
            *st = ProcessStatus::Exited(code);
            ...
            let Ok(callback_permit) = callback_slots.try_acquire_owned() else {
                warn!("session {sid_wait}: callback capacity reached; result remains available through portal_process");
                return;
            };
        ...
        debug!("spawned background session {} pid {}", session_id, pid);
    ...
    pub async fn poll(&self, session_id: &str, offset: u64, timeout_ms: u64) -> Result<PollResult> {
        ...
        loop {
            let (bytes, truncated, next, st, notify, idle_s, total_out, output_encoding) = {
                ...
                let s = guard
                    .get(session_id)
                    .ok_or_else(|| anyhow::anyhow!("Unknown or expired session: {}; outcome may be unknown after Portal restart", session_id))?;
    ...
    pub async fn log(&self, session_id: &str, offset: u64, limit: u64) -> Result<LogResult> {
        ...
        let guard = self.sessions.lock().await;
        let s = guard.get(session_id).ok_or_else(|| {
            anyhow::anyhow!(
                "Unknown or expired session: {}; outcome may be unknown after Portal restart",
                session_id
            )
        })?;
        let st = s.status.lock().await.clone();
    ...
    pub async fn write_stdin(&self, session_id: &str, data: &[u8]) -> Result<()> {
        ...
        let input = {
            let guard = self.sessions.lock().await;
            let s = guard.get(session_id).ok_or_else(|| {
                anyhow::anyhow!("Unknown or expired session; outcome may be unknown")
            })?;
            anyhow::ensure!(
        ...
        .await
        .map_err(|_| anyhow::anyhow!("stdin write timed out; some input may have been delivered"))?
    }

    pub async fn kill(&self, session_id: &str) -> Result<()> {
        validate_session_id(session_id)?;
        let (status, notify, cleanup_failed) = {
            let guard = self.sessions.lock().await;
            let s = guard.get(session_id).ok_or_else(|| {
                anyhow::anyhow!("Unknown or expired session; outcome may be unknown")
            })?;
            ...
            (s.status.clone(), s.notify.clone(), s.cleanup_failed.clone())
        };
        tokio::time::timeout(Duration::from_secs(8), async {
            loop {
                ...
                if matches!(*status.lock().await, ProcessStatus::Exited(_)) {
                    break;
                }
                notified.await;
            }
        ...
        .await
        .map_err(|_| anyhow::anyhow!("Process cleanup not confirmed; outcome may be unknown"))?;
        anyhow::ensure!(
            !cleanup_failed.load(Ordering::SeqCst),
            "Process cleanup failed; outcome may be unknown"
        );
        Ok(())
    }
