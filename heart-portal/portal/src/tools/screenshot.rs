...
pub async fn capture(config: &PortalConfig, arguments: Value) -> Result<Value> {
    ...
    // Capture into an OS-private temporary directory; publish through the same
    // confined atomic writer as file tools, never hand the user path to a child.
    let temporary = tempfile::tempdir()?;
    let capture_path = temporary.path().join("capture.png");
    ...
    run_capture(&capture_path, &region, display_idx).await?;

    let meta = tokio::fs::metadata(&capture_path)
        .await
        .map_err(|e| anyhow::anyhow!("Failed to stat screenshot {}: {}", path.display(), e))?;
    if meta.len() == 0 {
        anyhow::bail!(
            "Screenshot command created an empty file: {}",
            path.display()
        );
    }
    ...
    anyhow::ensure!(
        ...
        "Screenshot exceeds 10 MiB limit"
    );
    let mut bytes = Vec::new();
    tokio::fs::File::open(&capture_path)
        .await?
        .take(10 * 1024 * 1024 + 1)
    ...
    let dimensions = read_png_dimensions(&capture_path).await?;
    super::file::write_scoped(config, &path, bytes, false).await?;
...
async fn run_capture(path: &Path, region: &CaptureRegion, display_idx: Option<u64>) -> Result<()> {
    ...
    run_command("screencapture", command).await.map_err(|e| {
        if e.to_string().contains("not found") {
            anyhow::anyhow!(
                "screencapture not found; portal_screenshot requires macOS screencapture"
            )
        } else {
            e
        }
...
async fn run_capture(path: &Path, region: &CaptureRegion, display_idx: Option<u64>) -> Result<()> {
    ...
    run_command("powershell", command).await.map_err(|e| {
        if e.to_string().contains("not found") {
            anyhow::anyhow!(
                "powershell not found; portal_screenshot on Windows requires PowerShell"
            )
        } else {
            e
        }
...
async fn run_command(name: &str, mut command: Command) -> Result<()> {
    let output = crate::child_process::output(
        &mut command,
        super::text::OutputEncoding::Utf8,
        Duration::from_secs(30),
        16_384,
    )
    .await
    .map_err(|error| anyhow::anyhow!("Failed to run {}: {}", name, error))?;
