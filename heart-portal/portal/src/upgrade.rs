...
#[cfg(windows)]
pub(crate) async fn verify_local_windows_release(bytes: &[u8]) -> Result<String> {
    ...
    let version = release["tag_name"]
        ...
        .trim_start_matches('v');
    anyhow::ensure!(
        compare_versions(version, PORTAL_VERSION) == std::cmp::Ordering::Greater,
        "The latest official release must be newer than the installed Portal"
    );
    let platform = Platform {
        slug: "windows-x86_64".into(),
    };
...
mod tests {
    ...
    fn windows_release_requires_matching_asset_digest() {
        ...
        for invalid in [
            ...
            assert!(
                verify_release_digest(&invalid, &platform, bytes).is_err(),
                "{invalid}"
            );
        }
