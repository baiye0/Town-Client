...
fn make_launch(
    ...
    let (resolved, default_config) = if config_path.is_file() {
        ...
        defaults.tools.exec = false;
        defaults.tools.screenshot = false;
        defaults.tools.custom_tools_enabled = false;
        defaults.kits_enabled = false;
        let token = uuid::Uuid::new_v4().to_string();
        defaults.portal_mcp_token = Some(token.clone());
        (
            defaults,
            Value::String(format!(
                "portal_mcp_token = \"{token}\"\n{}",
                include_str!("../../portal.example.toml")
            )),
        )
    };
