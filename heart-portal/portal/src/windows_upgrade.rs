...
fn spawn_worker(worker: &Path, root: &Path, recover: bool) -> Result<DetachedWorker> {
    ...
    unsafe {
        ...
        Ok(DetachedWorker(OwnedHandle::from_raw_handle(
            process.hProcess,
        )))
    }
}
...
pub async fn upgrade_file(path: &Path) -> Result<()> {
    ...
    let file = tokio::fs::File::open(&path)
        .await
        .context("Opening downloaded upgrade")?;
    ...
    anyhow::ensure!(
        ...
        "Candidate executable is too large"
    );
    // Never execute a file to ask whether it is trusted. Verify these exact
    // bytes against official HTTPS release metadata before the worker sees it.
    let version = crate::upgrade::verify_local_windows_release(&bytes).await?;
