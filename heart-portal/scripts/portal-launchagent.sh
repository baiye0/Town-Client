...
# A fresh telemetry generation must not overwrite the upgrade transaction nonce.
HEART_PORTAL_STATUS_NONCE=$(/usr/bin/uuidgen)
HEART_PORTAL_STATUS_FILE="$root/.portal-connection-status.json"
printf '%s' "$HEART_PORTAL_STATUS_NONCE" > .portal-status-nonce.tmp
mv -f .portal-status-nonce.tmp .portal-status-nonce
export HEART_PORTAL_STATUS_NONCE HEART_PORTAL_STATUS_FILE
