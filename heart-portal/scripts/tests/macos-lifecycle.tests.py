        ...
        with tempfile.TemporaryDirectory(prefix='portal relay test ') as temporary, socket.socket() as relay:
            ...
            try:
                ...
                with client, stream:
                    ...
                    status_path = root / '.portal-connection-status.json'
                    wait_for(status_path.exists, 'structured connection status')
                    status = json.loads(status_path.read_text())
                    self.assertEqual(status['pid'], pid)
                    self.assertEqual(status['state'], 'connected')
                    self.assertEqual(status['nonce'], (root / '.portal-status-nonce').read_text().strip())
                    snapshot = subprocess.run([str(root / 'target/release/heart-portal'), 'status', '--json'], check=True, capture_output=True, text=True, timeout=10)
                    self.assertTrue(json.loads(snapshot.stdout)['fresh'])
