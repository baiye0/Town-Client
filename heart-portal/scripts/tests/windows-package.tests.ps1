...
try {
    ...
    if ($LocalOnly -and $LocalPort -ne 9100) {
        [IO.File]::WriteAllText((Join-Path $testRoot 'portal.toml'), "bind='127.0.0.1:$LocalPort'`nportal_mcp_token='fixture-local-token'`nworkspace='./workspace'`nkits_enabled=false`n")
    }
    ...
    if ($Candidate) {
        ...
        Assert ($result.code -ne 0) 'unpublished local build is rejected by the public upgrade entry'
        Assert (Test-PortalReady $testRoot $version) 'unverified candidate leaves the installed runtime alive'
        Write-Output 'PASS: public upgrade rejects unpublished local candidate'
    }
    # Trusted fixture handoff tests the recovery transaction separately from
    # release provenance. No production bypass is added for community builds.
        $stage = Join-Path $testRoot ('.portal-upgrades\' + [guid]::NewGuid().ToString('N'))
        [IO.Directory]::CreateDirectory($stage) | Out-Null
        $candidatePath = Join-Path $stage 'heart-portal.exe'
        $fixtureBinary = if ($Candidate) { $Candidate } else { $Binary }
        $fixtureVersion = if ($Candidate) { $nextVersion } else { $version }
        Copy-Item -LiteralPath $fixtureBinary -Destination $candidatePath
        ...
        $request = @{ root=$testRoot; target=$exe; candidate=$candidatePath; version=$fixtureVersion; sha256='invalid'; parent_pid=[int]::MaxValue; ack=(Join-Path $stage 'accepted.json'); error=(Join-Path $stage 'error.json') }
